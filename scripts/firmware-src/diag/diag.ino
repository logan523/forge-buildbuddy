...
// compile — never committed (see .gitignore). Only exists after that script
// has run at least once; a raw `arduino-cli compile` on a fresh checkout
// will fail here, which is intentional — always compile through the script.
#include "build_id.h"
...
void setup() {
  ...
  // First line printed, before anything that can stall (config read, I2C
  // init) — lets Forge's post-flash confirmation banner (flash-console.tsx)
  // read this back immediately and confirm the device is actually running
  // the build that was just flashed, not a stale one.
